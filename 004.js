...
export default function createStore(reducer, preloadedState, enhancer) {
  if (typeof preloadedState === 'function' && typeof enhancer === 'undefined') {
    ...
    so if the engine sees "no enhancer" but a preloadedState with the type function, 
    then it means the user meant the second argument as enhancer (and not as preloaded state)
    -----
    interestingly redux doesn'use default arguments here...
    ...maybe because the pattern we see instead is more powerful / simply is the only option when several arguments are optional? */
